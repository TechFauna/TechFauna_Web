...
const ControleReprodutivo = ({ user }) => {
  ...
  const handleReproduction = async () => {
    ...
    try {
      ...
      console.error("Erro ao iniciar reprodução:", err.message);
    }
  };
  ...
  const updateGestationStatus = () => {
    setGestations((prev) =>
      prev.map((gestation) => {
        const timeElapsed = Math.floor((new Date() - new Date(gestation.data_inicio)) / (1000 * 60 * 60 * 24));
        if (timeElapsed >= 5 && gestation.status === "Em andamento") {
          (async () => {
            try {
              ...
              await supabase
                .from("recintos")
                .update({ qnt_animais: parseInt(recinto.qnt_animais) + 1 })
                .eq("id_recinto", gestation.id_recinto);
            } catch (error) {
  ...
  useEffect(() => {
    const interval = setInterval(updateGestationStatus, 10000);
    ...
    <div className="controle-reprodutivo-container">
      ...
      <div className="form-group">
        ...
        <select value={selectedRecinto} onChange={(e) => setSelectedRecinto(e.target.value)}>
          ...
          {recintos.map((r) => <option key={r.id_recinto} value={r.id_recinto}>{r.nome}</option>)}
        </select>
      </div>
      ...
      <h2 style={{maxWidth:980, margin:'0 auto 10px'}}>Gestações Ativas</h2>
      <div className="active-gestations">
        {gestations.length ? gestations.map((g, i) => (
          <div key={i} className="gestation-card">
            <p><strong>Espécie:</strong> {species.find((s) => s.id === g.id_especie)?.name || "—"}</p>
            <p><strong>Recinto:</strong> {recintos.find((r) => r.id_recinto === g.id_recinto)?.nome || "—"}</p>
            <p><strong>Início:</strong> {new Date(g.data_inicio).toLocaleDateString()}</p>
            <p><strong>Status:</strong> {g.status}</p>
          </div>
        )) : <p style={{maxWidth:980, margin:'0 auto', color:'#5c7a68'}}>Nenhuma gestação ativa no momento.</p>}
      </div>
    </div>
  );
};
